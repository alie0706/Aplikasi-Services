...
class Transaksimasuk extends CI_Controller{
    ...
    function index()
    {
        ...
        $tgl2   =   $this->input->post('tgl12');
            ...
            $data['record'] = $this->model_transaksi_masuk->cari($tgl1,$tgl2);
// print_r('test');
// die;
            $this->template->load('template','laporan/transaksi_masuk/lihat_data',$data);
    ...
    function cari()
    {
            ...
            $record = $this->model_transaksi_masuk->cari($tgl1,$tgl2)->result();
            // print_r($record[0]->no_tanda_terima);
            // die;
            $data['tgl1']       =   $this->input->post('tgl1');
            $data['tgl2']       =   $this->input->post('tgl2');
            $data['record'] = $this->model_transaksi_masuk->cari($tgl1,$tgl2);
            // $data['recorddetail']     =  $this->model_transaksi_masuk_detail->get_one_transaksi($record[0]->id_transaksi);
// print_r($tgl2);
// die;
            $this->template->load('template','laporan/transaksi_masuk/lihat_data',$data);
...
// print_r($tgl2);
// die;
            $this->load->view('cetak/laporan_transaksi_masuk',$data);
